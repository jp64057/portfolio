...
// Which of the two DynamoDB calls is this? The résumé-count Query sets Select: COUNT.
const isCount = (cmd: any) => cmd.input?.Select === 'COUNT'
...
describe('computeStats', () => {
  it('aggregates page views and counts résumé downloads', async () => {
    mockSend.mockImplementation((cmd: any) => {
      if (isCount(cmd)) return Promise.resolve({ Count: 7 })
    ...
    expect(stats.resumeDownloads).toBe(7)
  ...
  it('follows Scan pagination across pages', async () => {
    mockSend.mockImplementation((cmd: any) => {
      if (isCount(cmd)) return Promise.resolve({ Count: 0 })
  ...
  it('handles empty data gracefully', async () => {
    mockSend.mockImplementation((cmd: any) =>
      Promise.resolve(isCount(cmd) ? { Count: 0 } : { Items: [] })
    )

    const stats = await computeStats()
    expect(stats.totalPageViews).toBe(0)
    expect(stats.pages).toEqual([])
    expect(stats.resumeDownloads).toBe(0)
  })
})
...
describe('handler', () => {
  ...
  it('caches results and sets Cache-Control (runs last — populates the cache)', async () => {
    mockSend.mockImplementation((cmd: any) =>
      Promise.resolve(isCount(cmd) ? { Count: 1 } : { Items: [{ PK: 'visitor_count::/', count: 3 }] })
    )
    ...
    // One Scan + one Query for the first call; the second is served from cache.
